...
func testUserRoleToOneRoleUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
...
func testUserRoleToOneSetOpRoleUsingUser(t *testing.T) {
	var err error
...
var (
	userRoleDBTypes = map[string]string{`ID`: `uuid`, `UserID`: `uuid`, `RoleID`: `uuid`, `CreateAt`: `timestamp with time zone`, `UpdateAt`: `timestamp with time zone`}
	_               = bytes.MinRead
)
