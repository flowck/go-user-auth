...
func testRoleToManyUserUserRoles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.UserUserRoles().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadUserUserRoles(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserUserRoles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserUserRoles = nil
	if err = a.L.LoadUserUserRoles(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserUserRoles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testRoleToManyAddOpUserUserRoles(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserUserRoles(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserUserRoles[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserUserRoles[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserUserRoles().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	roleDBTypes = map[string]string{`ID`: `uuid`, `Name`: `character varying`, `CreateAt`: `timestamp with time zone`, `UpdateAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
