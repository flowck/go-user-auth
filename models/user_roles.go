...
var UserRoleTableColumns = struct {
	...
	CreateAt: "user_roles.create_at",
	UpdateAt: "user_roles.update_at",
}
...
var UserRoleWhere = struct {
	...
	CreateAt: whereHelpertime_Time{field: "\"user_roles\".\"create_at\""},
	UpdateAt: whereHelpertime_Time{field: "\"user_roles\".\"update_at\""},
}
...
type userRoleR struct {
	...
	User *Role `boil:"User" json:"User" toml:"User" yaml:"User"`
}
...
var (
	userRoleAllColumns            = []string{"id", "user_id", "role_id", "create_at", "update_at"}
	userRoleColumnsWithoutDefault = []string{"user_id", "role_id"}
	userRoleColumnsWithDefault    = []string{"id", "create_at", "update_at"}
...
func (o *UserRole) User(mods ...qm.QueryMod) roleQuery {
	queryMods := []qm.QueryMod{
...
func (userRoleL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUserRole interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.UserUserRoles = append(foreign.R.UserUserRoles, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserID == foreign.ID {
				...
				foreign.R.UserUserRoles = append(foreign.R.UserUserRoles, local)
				break
			}
		}
	}
...
// Adds o to related.R.UserUserRoles.
func (o *UserRole) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Role) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
	...
	if related.R == nil {
		related.R = &roleR{
			UserUserRoles: UserRoleSlice{o},
		}
	} else {
		related.R.UserUserRoles = append(related.R.UserUserRoles, o)
	}
